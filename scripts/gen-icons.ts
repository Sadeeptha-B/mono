 ...
 *
 * The icon is the cat's head — the same crop the app header shows, drawn from
 * the same rows in `frames.ts`. Nothing is redrawn here, so the icon cannot
 * quietly stop being a picture of the thing in the corner of the app.
 *
 ...
 * script cannot import it: `cat.ts` reaches into the app's module graph, and
 * this stays a plain node script.
 */
